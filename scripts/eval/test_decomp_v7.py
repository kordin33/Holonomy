"""
test_decomp_v7.py - Test V7 Surgical Strike
"""
...
from deepfake_guard.features.holonomy_decomposition_v7 import HolonomyDecompositionV7
...
def main():
    print("="*70)
    print("🔬 TEST: DECOMPOSITION V7 - SURGICAL STRIKE")
    print("   Baseline + Multi-Commutator + Patch Disagreement")
    print("="*70)
    ...
    print("\n📊 Extracting V7...")
    v7_features = {
        'baseline': [], 'commutator': [], 'patch': [], 'all': []
    }
    
    for img in tqdm(images, desc="V7"):
        try:
            feats = v7.extract_features(encoder, img)
            for k in v7_features:
                v7_features[k].append(feats[k])
        except Exception as e:
            print(f"Error: {e}")
    
    for k in v7_features:
        v7_features[k] = np.array(v7_features[k], dtype=np.float32)
    ...
    # V7 Components
    for key in ['baseline', 'commutator', 'patch', 'all']:
        name = f"V7_{key}"
        res = test_stable(v7_features[key], labels, name)
