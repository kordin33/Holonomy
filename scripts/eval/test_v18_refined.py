"""
test_v18_refined.py - V18 PRO (Log-Space, Delta, Trimmed)
Sample: 100 images.
"""
...
from deepfake_guard.features.holonomy_v18_refined import HolonomyV18Refined
...
def main():
    print("="*70)
    print("🔬 TEST: V18 REFINED (Log-Space + Delta + TrimmedMean)")
    print("="*70)
    ...
    v18r = HolonomyV18Refined()
    feats = []
    
    print("Extracting...")
    for img in tqdm(images):
        feats.append(v18r.extract_features(encoder, img))
    
    feats = np.array(feats, dtype=np.float32)
    auc = test_auc(feats, labels)
    
    print(f"\n  V18 Refined (126D): {auc:.4f}")
    if auc > 0.90:
